...
library(data.table)
dataset_neg <- fread("./data/Movie Review/rt-polarity.neg", sep="\n", data.table=FALSE, col.names="review")
dataset_pos <- fread("./data/Movie Review/rt-polarity.pos", sep="\n", data.table=FALSE, col.names="review")
dataset <- rbind(dataset_neg, dataset_pos)
labels <- as.factor(c(rep("N",nrow(dataset_neg)), rep("P",nrow(dataset_pos))))
