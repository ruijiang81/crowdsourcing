...
expected_combinations <- 
    expand.grid(database_name = database_name_levles,
...
dataset <- 
    dataset %>%
    mutate_(x_axis = paste0("as.numeric(",x_axis,")"),
            y_axis = y_axis) %>%
    mutate(database_name = factor(database_name, levels = database_name_levles),
