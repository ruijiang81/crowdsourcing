ledger_feature_extractor <- function(ledger){
    ...
    assertive::assert_is_subset(                                
        c("repetition", "batch","payment_selected",
          "AUC_partial_train_set_1", "expected_performance_1"),
        colnames(ledger))
    ...
    AUC_expected <- (expected_performance_matrix * dummy_payment_matrix) %>% rowSums()
    #'
    ...
    ledger <-
        ledger %>% 
        mutate(AUC_partial_train_set = AUC_partial, AUC_expected = AUC_expected) %>%
