...
for(s in 1:nrow(param)){
    ...
    {
        cost_function_type = param[s,"primary_cost_function"]
        # Intialise payment counter
        payment_counter = data.frame(matrix(0,ncol=length(price_per_label_values)))
        colnames(payment_counter) = price_per_label_values
        ...
        tryCatch({
            ...
            if (num_batches_per_cost_initial_training_set>0){
                ...
                for (i in 1:num_price_per_label_values){
                    for (j in 1:num_batches_per_cost_initial_training_set){
                        ...
                        # Update counters #
                        ###################
                        ## updating the payment counter
                        payment_counter = payment_counter + 1
                        payment_counter[colnames(payment_counter) %in% pay_per_label] = 0
            ...
            while ((cost_so_far <= max_total_cost) & 
                ...
                ################################################################
                # (payment_counter) = price_per_label_values
                if(any(payment_counter>=max_period_per_price)){
                    # Choose the next payment
                    pay_per_label = as.numeric(names(which.max(payment_counter)))
                    # Increase payment counter
                    payment_counter = payment_counter + 1
                    # Reset the chosen payment
                    payment_counter[colnames(payment_counter) %in% pay_per_label] = 0
                } else {
                    # Choose the next payment
