...
param <- expand.grid(
    ...
    payment_selection_criteria=c("random", "min_pay_per_label", "max_pay_per_label",
                                 ...
                                 "always_0.02", "always_0.08", "always_0.14", "always_0.19", "always_0.25",
                                 "one_time_max_ratio")[c(13)],
...
for(s in 1:nrow(param)){
    ...
    for(counter_repetitions in 1:repeatitions)
    {
        ...
        tryCatch({
            ...
            ####################################################################
            #' Decide which price to pay for the rest of the simulation
            ####################################################################  
            if(payment_selection_criteria=="one_time_max_ratio")
